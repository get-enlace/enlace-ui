...
WorkflowNode {
  ...
  durationMs?: number
  checks?: AssertCheck[]    // "assert" kind only — same synthetic-node-only story as durationMs above
...
// attaches to the collection).
Preset {
  id: string                // unique within the collection's own `presets` array
  kind: "wait" | "assert"
  durationMs?: number       // "wait" kind only
  checks?: AssertCheck[]    // "assert" kind only — every check must pass or the preset (and collection) fails
}
