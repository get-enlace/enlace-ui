...
export class EncryptionUnavailableError extends Error {
  constructor(
    message = 'Encrypted collection export/import needs the Web Crypto API (`crypto.subtle`), which is unavailable in this environment.'
  ) {
    super(message);
...
/**
 * `crypto.subtle` is missing in some environments — notably browsers on
 * plain HTTP (not a secure context), and unusual/older runtimes. Check
 * explicitly and fail with a clear message rather than letting
 * `encryptCollection`/`decryptCollection` throw a raw `TypeError`.
 */
 ...
 * otherwise throw a raw `DOMException`/`InvalidCharacterError` out of
 * `atob`, so the whole body below is covered, not just the
 * `crypto.subtle.decrypt` call.
 */
